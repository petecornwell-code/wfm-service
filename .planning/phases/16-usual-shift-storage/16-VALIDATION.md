...
|----------|-------------|------------|--------|-------------------------|
| Generated template opens cleanly in Excel with a working Usual Shift dropdown | USHF-02 (D-10) | A POI round-trip test re-reads the file with the same library that wrote it; it cannot detect Excel-side corruption from the explicit-list 255-character limit | ⛔ **OPEN** — `16-UAT.md` test 3, `blocked_by: third-party` | Everything short of Excel itself is measured on the live deployed template: 7 `dataValidation` blocks at `O2:U1048576`, each `formula1` 104 chars against the 255 limit (151 spare), retired templates excluded, pre-fill round-trips. Remaining: open `/tmp/desk-template.xlsx` in real Excel, click a Usual Shift cell, confirm the dropdown lists live names and no repair prompt appears |
