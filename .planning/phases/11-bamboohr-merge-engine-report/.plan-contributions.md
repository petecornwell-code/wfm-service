# Plan:pre capability contributions (planner-targeted)

> Injected verbatim by the plan-phase orchestrator from the GSD capability registry.
> These are MANDATORY planning contributions — apply each in full when producing PLAN.md.


---

## Contribution: `ai-integration` (when: `workflow.api_coverage_gate`)
...
## Contribution: `assumption-delta` (when: `workflow.assumption_delta`)
...
## Contribution: `schema-gate` (when: `workflow.schema_push_detection`)
...
## Contribution: `security` (when: `workflow.security_enforcement`)

**Resolved config values (use these exact values):**
