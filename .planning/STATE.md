...
| deferred_items | 02/deferred-items.md (archived v1.0): AWS IAM access key CSV in sample-data. **NOT resolved — corrected 2026-09-21, after this row first understated it.** The *CSV file* is handled: absent from disk, never tracked, never committed, and `.gitignore:36` carries `*accessKeys*.csv`. But the **access key ID itself was committed in `d478251` (2026-04-03) and has been on the GitHub remote ever since**, inside the very deferred-item text describing the risk — roughly five months of exposure. Found when GitHub push protection blocked the v1.3 close: it flagged a *new* copy of the same ID that the close had written into this table (removed from the four unpushed commits by `filter-branch`; the bypass URL was not used). Push protection did not flag `d478251` only because it does not re-scan already-pushed content — not because that copy is safe. **Action required: rotate the key in AWS IAM and treat it as exposed.** Scrubbing it from history would mean rewriting a pushed commit; rotation is what actually closes this. The ID is in `.planning/milestones/v1.0-phases/02-security-cleanup-oidc-setup/deferred-items.md:7` and is deliberately not repeated here | acknowledged (understated — see above) | 2026-09-21 | v1.3 |
...
- **[Phase 14, CORRECTED 2026-09-03]** Shift templates ~~have **no delete endpoint**~~ — that was true as Phase 14 shipped (T-14-14), but **Phase 15's `81117e3` added `DELETE /desks/{deskId}/shift-templates/{id}`** and this entry was never updated. Current behaviour: the delete endpoint exists and is **guarded**, refusing with a 409 when the template is referenced by any `agent_shift_assignment` (Phase 15 guard) or any `agent_usual_shift` (Phase 16 plan 16-02 guard), with a message directing the operator to retire it instead; an unreferenced template is hard-deleted along with its break bands. Retirement (the `effective_to` edit) is still never blocked by references. Era identity is `(tenant_id, desk_id, name, effective_from)` unique **plus** a service-level same-name non-overlap check; the unique key alone permits overlapping ranges.
...
- [Phase 14]: [Phase 14]: Phase 14 Plan 03: updateShiftTemplate is the sole edit/retire mechanism (P-11) -- setting effectiveTo retires a template; no delete or retire method exists on ShiftTemplateService or ShiftTemplateController *(accurate as of Phase 14; SUPERSEDED by Phase 15's `81117e3`, which added deleteShiftTemplate to both -- left as written because this is a dated record of what Plan 03 decided, not a live constraint)*
