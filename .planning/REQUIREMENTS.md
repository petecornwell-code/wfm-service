...
| XCUT-01 (display verification) | Phases 14, 15, 16, 17 | Partial — Phases 14/15 complete; Phase 16's store → roster → export trace is proven end-to-end in one test (`UsualShiftTracerTest#happyPath_storeRosterExport_endToEnd`), but its frontend leg is human-verification-only (no frontend test framework exists in this project); Phase 17 plan 17-03 added the drift report's backend/export leg (`ScheduleDetailResponse.driftReport` + the Excel `Drift Report` sheet, headers byte-identical between the two surfaces); the frontend Drift Report tab is plan 17-05's outstanding obligation |
