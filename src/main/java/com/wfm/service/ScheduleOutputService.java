...
public class ScheduleOutputService {
    ...
    /**
     * Start-time spread, in minutes, that {@link #buildConsistencyReport} scores agents
     * against. Two hours, the target set by the consistency plan. A reporting threshold only —
     * the solver charges spread continuously and knows nothing about this value.
     */
    private static final int START_SPREAD_TARGET_MINUTES = 120;
    ...
    /**
     * 8.5 Consistency Report — per-agent start and break-offset spread across the schedule
     * period. Stage 3 of the consistency plan.
     *
     * <p>Stages 0 to 2 added constraints that shape how steady an agent's week is, but their
     * effect is only visible inside an aggregate soft score that conflates all fourteen
     * constraints. A solve could look better while consistency got worse. This reports the
     * quantity itself, in the same terms the plan's target is written in.
     *
     * <p>Mirrors the solver's definitions exactly so the report and the score cannot disagree:
     * a day's start is its earliest assigned slot, and a break offset is the distance from that
     * start to the first gap — see {@code ScheduleConstraintProvider.consistentDailyStart} and
     * {@code consistentBreakOffset}. Spread is reported in minutes rather than the solver's
     * whole increments, because this is for reading rather than for scoring.
     *
     * <p>Agents with no assignment at all are absent rather than present with a zero spread.
     * An agent who is not scheduled has no consistency to report, and counting them would
     * inflate every percentage here.
     */
    public ConsistencyReport buildConsistencyReport(Schedule schedule) {
        Map<UUID, Map<LocalDate, List<AgentAssignment>>> grouped = new HashMap<>();
        Map<UUID, String> agentNames = new HashMap<>();
        for (AgentAssignment a : schedule.getAssignments()) {
            if (a.getAgent() == null) continue;
            UUID agentId = a.getAgent().getId();
            agentNames.putIfAbsent(agentId, a.getAgent().getName());
            grouped
                    .computeIfAbsent(agentId, k -> new HashMap<>())
                    .computeIfAbsent(a.getTimeslot().getDate(), k -> new ArrayList<>())
                    .add(a);
        ...
        List<ConsistencyReportEntry> entries = new ArrayList<>();
        for (var agentEntry : grouped.entrySet()) {
            List<LocalTime> dayStarts = new ArrayList<>();
            List<Integer> breakOffsets = new ArrayList<>();

            for (var dateEntry : agentEntry.getValue().entrySet()) {
                List<AgentAssignment> dayAssignments = new ArrayList<>(dateEntry.getValue());
                dayAssignments.sort(Comparator.comparing(a -> a.getTimeslot().getStartTime()));

                LocalTime dayStart = dayAssignments.get(0).getTimeslot().getStartTime();
                dayStarts.add(dayStart);

                List<BreakDetail> breaks = findBreaks(dayAssignments);
                if (!breaks.isEmpty()) {
                    breakOffsets.add((int) ChronoUnit.MINUTES.between(
                            dayStart, breaks.get(0).startTime()));
                }
            }

            LocalTime earliest = Collections.min(dayStarts);
            LocalTime latest = Collections.max(dayStarts);
            Integer minOffset = breakOffsets.isEmpty() ? null : Collections.min(breakOffsets);
            Integer maxOffset = breakOffsets.isEmpty() ? null : Collections.max(breakOffsets);

            entries.add(new ConsistencyReportEntry(
                    agentEntry.getKey(),
                    agentNames.get(agentEntry.getKey()),
                    dayStarts.size(),
                    earliest,
                    latest,
                    (int) ChronoUnit.MINUTES.between(earliest, latest),
                    breakOffsets.size(),
                    minOffset,
                    maxOffset,
                    minOffset == null ? null : maxOffset - minOffset));
        }

        entries.sort(Comparator
                .comparing(ConsistencyReportEntry::agentName,
                        Comparator.nullsLast(Comparator.naturalOrder()))
                .thenComparing(ConsistencyReportEntry::agentId));

        return new ConsistencyReport(entries, summariseConsistency(entries));
    }

    private ConsistencySummary summariseConsistency(List<ConsistencyReportEntry> entries) {
        int totalAgents = entries.size();
        int singleDayAgents = (int) entries.stream().filter(e -> e.daysWorked() == 1).count();
        int identicalStart = (int) entries.stream().filter(e -> e.startSpreadMinutes() == 0).count();
        int withinTarget = (int) entries.stream()
                .filter(e -> e.startSpreadMinutes() <= START_SPREAD_TARGET_MINUTES).count();
        int maxStartSpread = entries.stream()
                .mapToInt(ConsistencyReportEntry::startSpreadMinutes).max().orElse(0);

        List<ConsistencyReportEntry> withBreaks = entries.stream()
                .filter(e -> e.breakOffsetSpreadMinutes() != null).toList();
        int identicalBreakOffset = (int) withBreaks.stream()
                .filter(e -> e.breakOffsetSpreadMinutes() == 0).count();
        int maxBreakSpread = withBreaks.stream()
                .mapToInt(ConsistencyReportEntry::breakOffsetSpreadMinutes).max().orElse(0);

        return new ConsistencySummary(
                totalAgents,
                singleDayAgents,
                identicalStart,
                percentageOf(identicalStart, totalAgents),
                withinTarget,
                percentageOf(withinTarget, totalAgents),
                START_SPREAD_TARGET_MINUTES,
                maxStartSpread,
                withBreaks.size(),
                identicalBreakOffset,
                percentageOf(identicalBreakOffset, withBreaks.size()),
                maxBreakSpread);
    }

    /**
     * {@code part} as a percentage of {@code whole}, to two decimal places. An empty population
     * reports 0%, not 100% — nothing was measured, and reporting perfection for an empty
     * schedule would be the more misleading of the two.
     */
    private BigDecimal percentageOf(int part, int whole) {
        if (whole <= 0) return BigDecimal.ZERO.setScale(2, RoundingMode.UNNECESSARY);
