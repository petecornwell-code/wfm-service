...
| The solver — read-path guard (Phase 17 plan 17-03, T-17-01/XCUT-02) | `SolverService#resolveUsualShiftTargets` | `com.wfm.service.SolverService` | Reads only the `List<AgentUsualShift>` its caller already fetched via the read finder in `startSolve`, and `UsualShiftResolutionService.resolve` — the method itself makes zero calls of any kind on `AgentUsualShiftRepository`. Proven two ways: behaviourally (a mocked `AgentUsualShiftRepository` receives zero mutating interactions, one explicit never-invoked assertion per mutating method the interface exposes, plus a no-further-interactions catch-all) and structurally (`SolverService.java`'s source, comments/javadoc stripped, contains no code line calling a mutating method on the `agentUsualShiftRepository` field anywhere in the class) | SolverUsualShiftWritePathGuardTest |
